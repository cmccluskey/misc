...
# Trim filename
def trim_filename(filename, length=1000):
  trimpoints=[]
  for match in re.finditer('\-|\_|\s', filename, ):
    trimpoints.append(match.start())
    print(trimpoints)
  middletrimpoint = len(trimpoints) // 2 
  print(middletrimpoint)
  # Make sure the string is sufficiently long so we don't loose too much
  if len(filename) >= length:
    if len(trimpoints) > 0:
    # Make sure the string is sufficiently long so we don't loose too much
      if float(trimpoints[middletrimpoint] / len(filename)) >= 0.35:
        for trimpoint in reversed(trimpoints):
          if len(filename[0:trimpoint]) <= length:
            return filename[0:trimpoint]
            break
        # Fall back to failsafe
...
parser.add_argument('-report', dest='report', action='store_true', default=False, help='Test and print report, do not touch files.')
parser.add_argument('-trim', dest='trim', action='store_true', default=False, help='Trim files to maxlength.')
parser.add_argument('--maxlength', dest='maxlength', action='store', default=135, help='Maximum size of filename component.')
...
for item in files:
  ...
  for exclude_string in exclude_strings:
    if exclude_string in item:
      logger.warning("Skipping file %s since exclude %s was present in the filename" % (item,exclude_string))
      pass
  original = item
  filename = None
  suffix = None
...
# 1K Trim filename to length
  filename = trim_filename(filename, int(args.maxlength))
  logger.debug("New File: %s" % filename)
